...
def launch_setup(context):
    ...
    launch_arguments={
        ...
        'gz_args': f'-r {world_file} --layers 1', 
    ...
    # 기존 RosGzBridge 부분을 지우고 아래 Node 방식으로 바꿉니다.
    ros_gz_bridge = Node(
        ...
        remappings=[
            # 가제보의 /camera/image를 ROS 2의 /camera/image_raw로 변경
            ('/camera/image', '/camera/image_raw'),
            # 조인트 상태 이름이 다를 경우 매핑 (필요시)
            ('/model/drobot/joint_state', '/joint_states'),
        ]
    )
    rviz2 = Node(
        package='rviz2',
